...
func init_gpu(num_cascades : int) -> void:
	...
	var spectrum_compute_shader := context.load_shader('res://engine/ocean/shaders/compute/spectrum_compute.glsl')
	var fft_butterfly_shader := context.load_shader('res://engine/ocean/shaders/compute/fft_butterfly.glsl')
	var spectrum_modulate_shader := context.load_shader('res://engine/ocean/shaders/compute/spectrum_modulate.glsl')
	var fft_compute_shader := context.load_shader('res://engine/ocean/shaders/compute/fft_compute.glsl')
	var transpose_shader := context.load_shader('res://engine/ocean/shaders/compute/transpose.glsl')
	var fft_unpack_shader := context.load_shader('res://engine/ocean/shaders/compute/fft_unpack.glsl')
	...
	unpack_sets.clear()
	for i in range(2):
		var displacement_map := context.create_texture(dims, RenderingDevice.DATA_FORMAT_R32G32B32A32_SFLOAT, RenderingDevice.TEXTURE_USAGE_STORAGE_BIT | RenderingDevice.TEXTURE_USAGE_SAMPLING_BIT | RenderingDevice.TEXTURE_USAGE_CAN_UPDATE_BIT | RenderingDevice.TEXTURE_USAGE_CAN_COPY_FROM_BIT, spectrum_layer_capacity)
		var normal_map := context.create_texture(dims, RenderingDevice.DATA_FORMAT_R32G32B32A32_SFLOAT, RenderingDevice.TEXTURE_USAGE_STORAGE_BIT | RenderingDevice.TEXTURE_USAGE_SAMPLING_BIT | RenderingDevice.TEXTURE_USAGE_CAN_UPDATE_BIT, spectrum_layer_capacity)
