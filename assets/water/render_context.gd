...
static func create(device : RenderingDevice=null) -> RenderingContext:
	...
	context.linear_sampler = context.deletion_queue.push(context.device.sampler_create(state))
	return context
...
func create_texture_set(texture_descriptor: Descriptor, sampler_rid: RID, shader: RID, set_index: int, binding: int) -> RID:
	var uniform := RDUniform.new()
	uniform.uniform_type = RenderingDevice.UNIFORM_TYPE_SAMPLER_WITH_TEXTURE
	uniform.binding = binding
	uniform.add_id(texture_descriptor.rid)
	uniform.add_id(sampler_rid)
	return deletion_queue.push(device.uniform_set_create([uniform], shader, set_index))
