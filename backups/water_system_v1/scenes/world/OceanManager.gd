...
@export var show_chunks: bool = false:
	set(value): show_chunks = value; _update_debug_grid()
...
func _center_on_camera() -> void:
	...
	var sx: float = round(tx / chunk_size) * chunk_size
	var sz: float = round(tz / chunk_size) * chunk_size
...
func _update_debug_grid() -> void:
	if not is_inside_tree():
		return
	for child in get_children():
		if child != null and is_instance_valid(child) and child.name == "DebugGrid":
			child.queue_free()
	if not show_chunks:
		return
	var half_size: float = chunk_size * 0.5 + chunk_overlap
	var center: int = (grid_size - 1) / 2
	var offset: float = (grid_size - 1) * chunk_size * -0.5
	var y: float = 0.5
	...
	for gx in range(grid_size):
		for gz in range(grid_size):
			var cx: float = gx * chunk_size + offset
			var cz: float = gz * chunk_size + offset
	...
	mi.material_override.albedo_color = Color.WHITE
