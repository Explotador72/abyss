"""
Wave system for ocean simulation.
Implements multi-octave Gerstner waves with group formation.
"""
...
@export var direction_noise: float = 0.4

@export var num_octaves: int = 3
@export var amplitude_falloff: float = 0.6
@export var frequency_mult: float = 2.0

@export var group_size: int = 2
@export var group_strength: float = 0.5
@export var group_frequency: float = 0.2
...
func initialize_wave_octaves() -> void:
	...
	var current_frequency: float = 1.0 / (wavelength_scale * 12.0)
...
func get_wave_height_at_position(pos: Vector3) -> float:
	...
	for i in range(wave_octaves.size()):
		...
		var qi: float = 1.0 / (k * wave_amplitude * 4.0 + 0.001)
		var qi_clamped: float = min(qi, 0.6)
