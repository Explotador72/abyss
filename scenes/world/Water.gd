...
func _update_debug() -> void:
	...
	while idx < get_child_count():
		if get_child(idx).name == "DebugOutline":
			get_child(idx).free()
		else:
			idx += 1
	if not show_chunks:
	...
	var half: float = chunk_size * 0.5 + chunk_overlap
	var y: float = 0.5
	var corners := [Vector3(-half, y, -half), Vector3(half, y, -half), Vector3(half, y, half), Vector3(-half, y, half)]
	for c in range(4):
		st.add_vertex(corners[c])
		st.add_vertex(corners[(c + 1) % 4])
